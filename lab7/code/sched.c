...
static pqueue_t in_queue; 	// Queue for incomming processes
static pqueue_t ready_queue;// Queue for ready processes
	...
	while (!load_done || !empty(&ready_queue)) {
		...
		if (proc == NULL) {
			...
			*/
			...
			// TODO: put its PCB back to the queue.
